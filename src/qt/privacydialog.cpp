                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBETEX ought to be enough for anybody." - Bill Gates, 2017
    ui->zBETEXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBETEXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBETEX->setEnabled(false);
        ui->pushButtonMintzBETEX->setToolTip(tr("zBETEX is currently disabled due to maintenance."));

        ui->pushButtonSpendzBETEX->setEnabled(false);
        ui->pushButtonSpendzBETEX->setToolTip(tr("zBETEX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBETEXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBETEX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBETEX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBETEX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBETEX in ") + 
...
void PrivacyDialog::on_pushButtonSpendzBETEX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBETEX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBETEX
        sendzBETEX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBETEX
    sendzBETEX();
}

void PrivacyDialog::on_pushButtonZBetexControl_clicked()
{
    ZBetexControlDialog* zBetexControl = new ZBetexControlDialog(this);
    zBetexControl->setModel(walletModel);
    zBetexControl->exec();
}

void PrivacyDialog::setZBetexControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBetexSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBETEX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Betex Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBETEXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBETEX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BETEX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBETEX</b>";
    ...
    // use mints from zBetex selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBetexControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBetexControlDialog::GetSelectedMints();
    }
    ...
    // Clear zbetex selector in case it was used
    ZBetexControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBetex Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Betex, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBetex Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBETEX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBETEX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBETEX "));
    ui->labelzBETEXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzBETEXSyncStatus->setVisible(fShow);
}
