...
#include "ui_zbetexcontroldialog.h"
...
std::list<std::string> ZBetexControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBetexControlDialog::listMints;

ZBetexControlDialog::ZBetexControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBetexControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBetexControlDialog::~ZBetexControlDialog()
{
    delete ui;
}

void ZBetexControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBetexControlDialog::updateList()
{
...
void ZBetexControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBetexControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBetex_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBetexControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBetexControlDialog::GetSelectedMints()
{
...
void ZBetexControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
